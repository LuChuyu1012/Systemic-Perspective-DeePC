...
for kcase = 1:numel(use_cases)
    ...
    fname_big = fullfile(data_dir, sprintf('s%02d_r%02d_case%02d.mat', ...
        sysnum, f_target, si));

    if ~isfile(fname_big)
        fprintf('Skip (file not found): case=%d\n', si);
        continue;
    end
    ...
    fprintf('case=%d | y used %d/%d runs | Ny=%d\n', si, nYeff, nruns, Ny);
    if hasU
        fprintf('case=%d | u used %d/%d runs | Nu=%d\n', si, nUeff, nruns, Nu);
    else
        fprintf('case=%d | u not found (no u_seq_opt in file)\n', si);
    end

    figy = figure('Color','w','Visible','on', ...
        'Name', sprintf('Output mean and CI95 case=%d', si));
...
%         basey = sprintf('MeanTraj_CI95_y_sys%02d_ft%02d_case%02d', ...
%             sysnum, f_target, si);
