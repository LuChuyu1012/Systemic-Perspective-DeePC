...
function [u_seq, y_seq,x_seq,y_pred_seq,coverage,g_seq,y_ini_seq,u_ini_seq,y_seq_all] = run_DeePC(U_p, Y_p, U_f, Y_f, desired_trajectory, T_ini, N, Q, R,x0, T,sys,randseed) 
rng(randseed);
    ...
    % 1. Initialize past input and output data
     ...
     u_ini=2*rand(1, T_steady_state)'-1;

    y_ini = zeros(T_steady_state, 1);  

    % Compute initial y_ini from system
    x_tmp = x0;                 
    for i = 1:T_steady_state
    ...
    for t = 1:T
        ...
        [g_opt, y_pred,u_opt] = solve_DeePC(U_p, Y_p, U_f, Y_f, u_ini, y_ini, r, Q, R);
        y_pred_seq(t,:) = y_pred';
        y_ini_seq(t,:) = y_ini';    
        u_ini_seq(t,:) = u_ini';
